...
import com.gok.pboot.enumeration.DataScopeType;
import com.gok.pboot.utils.SecurityUtils;
...
public abstract class BaseBizService {
    ...
    protected String dataScopeFilter(String bizAlias, String unitColumn, String userColumn) {
        AccessTokenResp user = SecurityUtils.getBaseUser();
        ...
        for(Object obj : user.getGroups()) {
            AccessGroupResp group = (AccessGroupResp)obj;
        ...
        if(DataScopeType.SCOPE_ALL.getValue().equals(dataScopeValue)){
            ...
            sqlString.append(" WHERE ooU.org_code LIKE '").append((user.getUnit()).getOrgCode()).append("%'");
            sqlString.append(" AND ").append(orgWhere).append(")");
        } else if (DataScopeType.SCOPE_ORG.getValue().equals(dataScopeValue)){
            sqlString.append(" EXISTS (SELECT 1 FROM tb_unit ooU");
            sqlString.append(" WHERE ooU.id = ").append((user.getUnit()).getId());
            sqlString.append(" AND ").append(orgWhere).append(")");
    ...
    protected boolean isSuperAdmin() {
        if (SecurityUtils.getBaseUser() == null){
            return false;
        } else {
            return BaseConfig.getAdminId()
                    .equals(SecurityUtils.getBaseUser().getId());
        }
    }
}
